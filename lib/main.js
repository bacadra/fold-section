...
export default {
  ...
  getReHead() {
    ...
    if (['source.python', 'source.cython'].includes(scopeName)) {
    ...
    } else if (['text.tex.latex'].includes(scopeName)) {
      var {reHead} = require('./latex-model');

    } else if (['text.blg'].includes(scopeName)) {
      var {reHead} = require('./bacadra-log-model');

    } else if (['text.bibtex'].includes(scopeName)) {
      var {reHead} = require('./bib-model');
