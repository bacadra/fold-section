'use babel';

import { CompositeDisposable } from 'atom';

let subscriptions

export default {

  activate(state) {
    subscriptions = new CompositeDisposable();

    subscriptions.add(atom.commands.add('atom-text-editor', {
      'bacadra-fold:fold-current-lvl': () => this.foldLvl(true),
      'bacadra-fold:unfold-current-lvl': () => this.unfoldCurrent(),
      'bacadra-fold:toggle-current-lvl': () => this.toggleCurrentLvl(),
      'bacadra-fold:fold-TOC': () => this.foldTOC(),
      'bacadra-fold:unfold-all': () => this.unfoldAll(),
      'bacadra-fold:fold-all-lvl-1': () => this.foldAllLvl(1),
      'bacadra-fold:fold-all-lvl-2': () => this.foldAllLvl(2),
      'bacadra-fold:fold-all-lvl-3': () => this.foldAllLvl(3),
      'bacadra-fold:fold-all-lvl-4': () => this.foldAllLvl(4),
      'bacadra-fold:fold-all-lvl-5': () => this.foldAllLvl(5),
      'bacadra-fold:fold-all-lvl-6': () => this.foldAllLvl(6),
      'bacadra-fold:fold-all-lvl-7': () => this.foldAllLvl(7),
      'bacadra-fold:fold-all-lvl-8': () => this.foldAllLvl(8),
      'bacadra-fold:fold-all-lvl-9': () => this.foldAllLvl(9),
      'bacadra-fold:fold-lvl-1': () => this.foldLvl(1),
      'bacadra-fold:fold-lvl-2': () => this.foldLvl(2),
      'bacadra-fold:fold-lvl-3': () => this.foldLvl(3),
      'bacadra-fold:fold-lvl-4': () => this.foldLvl(4),
      'bacadra-fold:fold-lvl-5': () => this.foldLvl(5),
      'bacadra-fold:fold-lvl-6': () => this.foldLvl(6),
      'bacadra-fold:fold-lvl-7': () => this.foldLvl(7),
      'bacadra-fold:fold-lvl-8': () => this.foldLvl(8),
      'bacadra-fold:fold-lvl-9': () => this.foldLvl(9),
    }));
  },

  deactivate() {
    subscriptions.dispose();
  },

  getReHead() {
    const editor = atom.workspace.getActiveTextEditor()
    scopeName = editor.getRootScopeDescriptor().scopes[0]

    if (['source.python'].includes(scopeName)) {
      var {reHead} = require('./python-model');

    } else if (['text.md'].includes(scopeName)) {
      var {reHead} = require('./mkdown-model');

    } else {
      atom.notifications.addError('Grammar "'+scopeName+'" not supported!')
      return
    }

    return reHead
  },

  foldLvl(lvlFold=true) {
    // console.log('***** fold START *****')

    const editor = atom.workspace.getActiveTextEditor()

    curPos  = editor.getCursorBufferPosition()
    currRow = curPos.row
    lastRow = editor.getLastBufferRow()

    reHead = this.getReHead()
    if (reHead == null) {return}
    if (lvlFold>reHead.length) {return}

    // empty start row variable
    let startRow, endRow, lvlHead

    if (lvlFold===true) {

      // look for start line
      for (let i = currRow; i >= 0; i--) {
        lineText = editor.lineTextForBufferRow(i)

        for (let lvl = 0; lvl < reHead.length; lvl++) {
          re = reHead[lvl]

          // console.log(i, lvl, 're:', re)

          if (re.test(lineText)) {
            startRow = i;
            lvlHead  = lvl;
            // console.log('finded')
            break
          }
        }

        if (startRow>=0) {break}

      }

    } else {
      lvlFold = lvlFold-1

      exitQ = false

      // look for start line
      for (let i = currRow; i >= 0; i--) {
        lineText = editor.lineTextForBufferRow(i)

        for (let lvl = 0; lvl <= lvlFold; lvl++) {
          re = reHead[lvl]

          // console.log(i, lvl, 're:', re)

          if (re.test(lineText)) {
            if (lvl!=lvlFold) {
              exitQ = true
              break
            }
            startRow = i;
            lvlHead  = lvl;
            // console.log('finded')
            break
          }
        }

        if (startRow>=0 || exitQ==true) {break}

      }

    }

    // console.log('startRow:', startRow)
    // console.log('lvlHead:', lvlHead)

    if (startRow==null) return

    // look for end line
    for (let i = currRow+1; i <= lastRow; i++) {
      lineText = editor.lineTextForBufferRow(i)


      for (let lvl = 0; lvl <= lvlHead; lvl++) {
        re = reHead[lvl]

        // console.log(i, lvl, 're:', re)

        if (re.test(lineText)) {
          endRow = i;
          // console.log('finded')
          break
        }
      }

      if (endRow>=0) {break}
    }

    // console.log('endRow:', endRow)

    if (!endRow) {endRow = lastRow} else {endRow=endRow-1}

    // console.log('startRow:', startRow)
    // console.log('endRow:', endRow)

    this.foldRange(startRow, endRow)

    editor.setCursorBufferPosition(curPos)

    // console.log('***** fold END *****')
  },


  foldRange(startRow, endRow) {
    const editor = atom.workspace.getActiveTextEditor()
    editor.setSelectedBufferRange(
      [[startRow, 1e10], [endRow, 1e10]]
    )
    editor.foldSelectedLines()
  },

  unfoldCurrent () {
    const editor = atom.workspace.getActiveTextEditor()
    const currentRow = editor.getCursorBufferPosition().row
    editor.unfoldBufferRow(currentRow)
  },

  toggleCurrentLvl() {
    const editor = atom.workspace.getActiveTextEditor()
    const currentRow = editor.getCursorBufferPosition().row
    if (editor.isFoldedAtBufferRow(currentRow)){
      this.unfoldCurrent()
    } else {
      this.foldLvl()
    }
  },

  unfoldAll() {
    const editor = atom.workspace.getActiveTextEditor()
    let lrow = editor.getLastBufferRow()
    for (let row = 0; row < lrow; row++) {
      editor.unfoldBufferRow(row)
    }
  },

  foldAllLvl(lvlFold) {
    // console.log('***** foldAll START *****')

    this.unfoldAll()

    const editor = atom.workspace.getActiveTextEditor()

    curPos  = editor.getCursorBufferPosition()
    lastRow = editor.getLastBufferRow()

    reHead = this.getReHead()
    if (reHead == null) {return}
    if (lvlFold>reHead.length) {return}

    lvlFold = lvlFold-1

    for (let i = 0; i <= lastRow; i++) {

      // console.log(i)

      lineText = editor.lineTextForBufferRow(i)

      re = reHead[lvlFold]

      if (re.test(lineText)) {

        startRow = i

        exitQ = false

        i++;

        // console.log('startRow:', startRow)

        for (let j = 0; i <= lastRow; i++) {

        // console.log(i)

          lineText = editor.lineTextForBufferRow(i)

          for (let lvl = 0; lvl <= lvlFold; lvl++) {
            re = reHead[lvl]

            if (re.test(lineText) || i==lastRow) {
              endRow = i;
              if (i==lastRow) {endRow++}

              // console.log('endRow:', endRow)

              this.foldRange(startRow, endRow-1)
              exitQ = true
              i--;
              break
            }
          }
          if (exitQ==true) {break}
        }

      }

    }

    editor.setCursorBufferPosition(curPos)

    // console.log('***** foldAll END *****')
  },


  foldTOC() {
    // console.log('***** foldTOC START *****')

    this.unfoldAll()

    const editor = atom.workspace.getActiveTextEditor()

    curPos  = editor.getCursorBufferPosition()
    lastRow = editor.getLastBufferRow()

    reHead = this.getReHead()
    if (reHead == null) {return}

    let startRow = -1

    for (let i = startRow+1; i <= lastRow; i++) {
      // console.log(i)
      lineText = editor.lineTextForBufferRow(i)
      for (let lvl = 0; lvl < reHead.length; lvl++) {
        re = reHead[lvl]
        if (re.test(lineText)) {
          // console.log('range:', startRow, i-1)
          this.foldRange(startRow, i-1)
          startRow = i
        } else if (i==lastRow) {
          // console.log('range end:', startRow, i)
          this.foldRange(startRow, i)
          break
        }
      }
    }

    editor.setCursorBufferPosition(curPos)

    // console.log('***** foldTOC END *****')
  },

};
